<template>
	<view class="bg-[var(--page-bg-color)] min-h-[100vh] order-list" :style="themeColor()">
		...
		<mescroll-body ref="mescrollRef" top="114rpx" @init="mescrollInit" @down="downCallback" @up="getBarberOrderListFn">
				...
				<template v-for="(item,index) in list" :key="index" >
					<view class="mb-[30rpx] bg-[#fff] p-[30rpx] rounded-[20rpx] mx-[30rpx]">
						...
						<view class="order-goods-item flex justify-between flex-wrap" @click="toLink(item)">
							...
							<view class="ml-[20rpx] flex flex-1 flex-col justify-between">
								...
								<view class="flex" @click.stop="callPhone(item.member.phone)">
									<u-avatar :src="img(item.member.headimg)" size="32" leftIcon="none" :default-url="img('static/resource/images/default_headimg.png')" />
									<view class="ml-[10rpx] leading-[70rpx] h-[70rpx]">{{item.member.nickname}}</view>
						...
						<view class="mt-[34rpx] flex justify-end z-10 ">
							...
							<view class="inline-block text-[26rpx] leading-[56rpx] px-[30rpx] border-[3rpx] border-solid border-[var(--primary-color)] rounded-full text-[#fff] ml-[20rpx] bg-[var(--primary-color)]" v-if="item.status == 1" @click="orderBtnFn(item, 'service')">开始服务</view>
							<view class="inline-block text-[26rpx] leading-[56rpx] px-[30rpx] border-[3rpx] border-solid border-[#999] rounded-full ml-[20rpx]" v-if="item.status == 2" @click="orderBtnFn(item, 'revert')">退回排队</view>
							
							<view class="inline-block text-[26rpx] leading-[56rpx] px-[30rpx] border-[3rpx] border-solid border-[var(--primary-color)] rounded-full text-[#fff] ml-[20rpx] bg-[var(--primary-color)]" v-if="item.status == 2" @click="orderBtnFn(item, 'finish')">服务完成</view>
						</view>
					</view>
				</template>
			</view>
...
import { getBarberOrderList, barberOrderCancel, barberOrderService, barberOrderRevert, barberOrderFinish } from '@/addon/zzhc/api/merchant';
...
let orderState = ref(1)
let orderStateList = ref([]);


onLoad((option) => {
	orderState.value = option.status || 1;
	getOrderStatusFn();
});

const getBarberOrderListFn = (mescroll) => {
	loading.value = false;
	let data: object = {
		page: mescroll.num,
		limit: mescroll.size,
	...
	getBarberOrderList(data).then((res) => {
...
const toLink = (data: any) => {
	redirect({ url: '/addon/zzhc/pages/merchant/barber/order/detail', param: { order_id: data.order_id } })
}
	...
	uni.showModal({
		...
		success: res => {
			if (res.confirm) {
				...
				barberOrderCancel(param).then((data) => {
					getMescroll().resetUpScroll();
				})
			}
		}
...
const finish = (item: any) => {
	uni.showModal({
		...
		success: res => {
			if (res.confirm) {
				let param = {order_id:item.order_id,store_id:currStoreId()};
				barberOrderFinish(param).then((data) => {
					getMescroll().resetUpScroll();
				})
			}
		}
...
const service = (item: any) => {
	let param = {order_id:item.order_id,store_id:currStoreId()};
	barberOrderService(param).then((data) => {
		getMescroll().resetUpScroll();
	})
}
...
const revert = (item: any) => {
	uni.showModal({
		...
		success: res => {
			if (res.confirm) {
				let param = {order_id:item.order_id,store_id:currStoreId()};
				barberOrderRevert(param).then((data) => {
					getMescroll().resetUpScroll();
				})
			}
		}
