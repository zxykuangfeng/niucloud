		...
		<view v-if="!loading" class="pb-20rpx">
			...
			<view class="flex z-2 justify-between items-center bg-[#fff] fixed left-0 right-0 bottom-0 min-h-[100rpx] px-1 flex-wrap  pb-[env(safe-area-inset-bottom)]">
				...
				<view class="flex justify-end mr-[30rpx]">
					...
					<view class="inline-block text-[26rpx] leading-[56rpx] px-[30rpx] border-[3rpx] border-solid text-[#fff] bg-primary border-primary rounded-full ml-[20rpx]" @click="orderBtnFn('service')" v-if="detail.status == 1">开始服务</view>
					<view class="inline-block text-[26rpx] leading-[56rpx] px-[30rpx] border-[3rpx] border-solid border-[#999] rounded-full" v-if="detail.status == 2" @click="orderBtnFn('revert')">退回排队</view>
					<view class="inline-block text-[26rpx] leading-[56rpx] px-[30rpx] border-[3rpx] border-solid text-[#fff] bg-primary border-primary rounded-full ml-[20rpx]" @click="orderBtnFn('finish')" v-if="detail.status == 2">服务完成</view>
				</view>
			</view>
		</view>
...
import { getBarberOrderDetail,barberOrderCancel,barberOrderService,barberOrderFinish,barberOrderRevert } from '@/addon/zzhc/api/merchant';
...
const orderDetailFn = () => {
	...
	getBarberOrderDetail(data).then((res) => {
...
const orderBtnFn = (type = '') => {
	if (type == 'back') {
		redirect({ url: '/addon/zzhc/pages/merchant/barber/order/list' })
	}else if(type == 'cancel') {
		cancel(detail.value);
