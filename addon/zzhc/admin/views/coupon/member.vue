        ...
        <el-card class="box-card !border-none" shadow="never">
            ...
            <el-card class="box-card !border-none my-[10px] table-search-wrap" shadow="never">
                <el-form :inline="true" :model="memberTable.searchParam" ref="searchFormRef">
                    <el-form-item :label="t('name')" prop="name">
                        <el-input v-model="memberTable.searchParam.name" :placeholder="t('namePlaceholder')" />
                    </el-form-item>
            ...
            <div class="mt-[10px]">
                <el-table :data="memberTable.data" size="large" v-loading="memberTable.loading">
                    ...
                    <el-table-column prop="atleast" :label="t('atleast')" min-width="120" :show-overflow-tooltip="true"/>
                    
					
					<el-table-column prop="expire_time" :label="t('expireTime')" min-width="120">
					    <template #default="{ row }">
							{{row.expire_time ? row.expire_time : '永久'}}
					    </template>
					</el-table-column>
					
					<el-table-column prop="use_time" :label="t('useTime')" min-width="120">
					    <template #default="{ row }">
							{{row.use_time ? row.user_time : '未使用'}}
					    </template>
					</el-table-column>
...
import { getMemberList, deleteMember } from '@/addon/zzhc/api/coupon'
import { img } from '@/utils/common'
import { ElMessageBox,FormInstance } from 'element-plus'
import Edit from '@/addon/zzhc/views/coupon/components/member-edit.vue'
